...
const getUserInfo = async (token) => new Promise((resolve, reject$$1) => {
    resolve({
        type : [400]
    });
});
...
const URL$2 = `http://34.67.150.50:8080`;

const resolvers$1 = {
	Query: {
		allReservations: (_, { }, ctx) =>
			getUserInfo(ctx).then(user => {
				console.log("ALL RESERVATIONS1 " + JSON.stringify(user.type && (user.type[0] != "500")) );
				if (user.type && (user.type[0] != "500")) {
					console.log("ALL RESERVATIONS111");
		...
		allCancelledReservations: (_, { }, ctx) =>
			getUserInfo(ctx).then(user => {
				if (user.type && (user.type[0] != "500"))
					return getRequest(`${URL$2}/reservations/cancelled`, '');
				return user
			}).catch(err => err)
		,
		allUserCancelledReservations: (_, { id }, ctx) =>
			getUserInfo(ctx).then(user => {
				if (user.type && (user.type[0] != "500"))
					return getRequest(`${URL$2}/reservations/cancelled/${id}`, '');
				return user
			}).catch(err => err)
		,
		allUserPendingReservations: (_, { id }, ctx) =>
			getUserInfo(ctx).then(user => {
				if (user.type && (user.type[0] != "500"))
					return getRequest(`${URL$2}/reservations/pending/${id}`, '');
				return user
			}).catch(err => err)
		,
		cancelledReservationsByExcursion: (_, { id }, ctx) =>
			getUserInfo(ctx).then(user => {
				if (user.type && (user.type[0] != "500"))
					return getRequest(`${URL$2}/reservations/cancelled/excursion-package/${id}`, '');
				return user
			}).catch(err => err)
		,
		allReservationsByUser: (_, { id }, ctx) =>
			getUserInfo(ctx).then(user => {
				if (user.type && (user.type[0] != "500"))
					return getRequest(`${URL$2}/reservations/${id}`, '');
				return user
			}).catch(err => err)
		,
	...
	Mutation: {
		cancelReservation: (_, { id }, ctx) =>
			getUserInfo(ctx).then(user => {
				if (user.type && (user.type[0] != "500"))
					return generalRequest(`${URL$2}/reservations/cancel/${id}`, 'PATCH');
				return user
			}).catch(err => err)
		,
		...
		deleteReservation: (_, { id }, ctx) =>
			getUserInfo(ctx).then(user => {
				if (user.type && (user.type[0] != "500"))
					return generalRequest(`${URL$2}/reservations/${id}`, 'DELETE');
				return user
			}).catch(err => err)
		,
		updateReservation: (_, { reservation }, ctx) =>
			getUserInfo(ctx).then(user => {
				if (user.type && (user.type[0] != "500"))
					return generalRequest(`${URL$2}/reservations/${id}`, 'PATCH', reservation);
				return user
			}).catch(err => err)
	}
};
...
const url$2 = '54.190.208.244';
const port$2 = 8000;
const entryPoint$1 = 'excursions';
const entryPoint1 = 'packages';

const URL$3 = `http://${url$2}:${port$2}/${entryPoint$1}`;
const URL1 = `http://${url$2}:${port$2}/${entryPoint1}`;
...
const url$3 = '18.236.1.65';
const port$3 = 3000;
const entryPoint$2 = 'suppliers';
const entryPoint2 = 'accounts';

const URL$4 = `http://${url$3}:${port$3}/${entryPoint$2}`;
const URL2 = `http://${url$3}:${port$3}/${entryPoint2}`;
...
const url$5 = process.env.LOGIN_URL || "3.13.112.89";
const port$5 = process.env.LOGIN_PORT || 3000;
const entryPoint$4 = process.env.LOGIN_ENTRY || "/sign_in";
const signUpEntryPoint = process.env.SIGN_UP_ENTRY || "/sign_up";

var jwt$1 = require('jsonwebtoken');
const URL$6 = `http://${url$5}:${port$5}/${entryPoint$4}`;
const signUpURL = `http://${url$5}:${port$5}/${signUpEntryPoint}`;
