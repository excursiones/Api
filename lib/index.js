...
const url$1 = '54.190.208.244';
const port$1 = 8000;
const entryPoint$1 = 'excursions';
const entryPoint1 = 'packages';

const URL$3 = `http://${url$1}:${port$1}/${entryPoint$1}`;
const URL1 = `http://${url$1}:${port$1}/${entryPoint1}`;

const resolvers$2 = {
	Query: {
		allExcursions: (_, { }, ctx) => {

			return getRequest(`${URL$3}/get`, '')
		},
		excursionById: (_, { id }) =>
	...
	Mutation: {
		createExcursion: (_, { excursion }) =>
			generalRequest(`${URL$3}/create`, 'POST', excursion),
		updateExcursion: (_, { id, excursion }) =>
			generalRequest(`${URL$3}/edit/${id}`, 'PUT', excursion),
		deleteExcursion: (_, { id }) =>
			generalRequest(`${URL$3}/delete/${id}`, 'DELETE'),
...
const TVEP_URL = `http://${url$3}:${port$3}/${tokenValidationEntryPoint}`;
...
const URL$4 = `http://${url$2}:${port$2}/${entryPoint$2}`;
const URL2 = `http://${url$2}:${port$2}/${entryPoint2}`;
